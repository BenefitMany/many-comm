...
module ApiListMethods
  ...
  def api_list_render(objs, options = {})
    ...
    objs = apply_filters(objs) unless @filters.blank?
    objs = objs.search_for(@search) unless @search.blank?
    collection =
      if @paginate
        objs.page(@page).per(@per_page).order(order_by.join(', ')) # kaminari
      else
  ...
  def order_by
    ...
    order << 'id ASC' if @sort.to_s != 'id' && enable_id_order
    order
  end
